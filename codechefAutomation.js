...
const scrollYAxis = async function (page, selector, yDistance) {
...
const solveTheGivenQuestion = async function (currPage, questionUrl, solution) {
    try {
        ...
        /*// If we want to compile the code:
            await waitAndClick(currPage, '._btn__text_17wjk_82');
            await currPage.waitForSelector('._status__container_vm234_18'); This element gets generated only when a status popup arrives after Compilation:
        */

        await waitAndClick(currPage, '#submit_btn');

        // We'll wait for the result to appear. But if CodeChef pops up some modal after submission of the question, the following program would crash. Hence:
        await Promise.race([
            currPage.waitForSelector('._run-result_10alf_2', { visible: true }),
            new Promise((resolve, _) => {
                // setTimeout(() => {
                //     resolve();      // To return a resolved promise.
                // }, 15000);
                setTimeout(resolve, 15000); // Can avoid the magic number. The value is big cuz we want to wait until CodeChef runs all of it's test cases for the submitted solution.
            }),
        ]);
...
const codechefAutomation = async function (dataArr) {
    try {
        ...
        // As the password field isn't visible when typing:
        await scrollYAxis(page, '#ajax-login-form #edit-name');
...
/*
//To scroll down untill we can't anymore:
await currTab.evaluate(async () => {
